...
export function hasPermission(
  userRole: UserRole,
  permission: keyof (typeof USER_ROLE_PERMISSIONS)[UserRole],
): boolean {
...
export function transformApiUser(apiUser: any): User {
  ...
  const userAdminRoles =
    apiUser.roles?.filter((role: string) => adminRoles.includes(role)) || [];
  ...
  if (userAdminRoles.length > 0) {
    ...
    effectiveRole =
      (rolePriority.find((role) =>
        userAdminRoles.includes(role),
      ) as UserRole) || apiUser.currentRole;
  }
