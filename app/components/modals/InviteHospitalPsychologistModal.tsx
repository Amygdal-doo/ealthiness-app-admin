...
export const InviteHospitalPsychologistModal: React.FC<
  InviteHospitalPsychologistModalProps
> = ({ isOpen, onClose, hospitalId, hospitalName }) => {
  ...
  const addPsychologistMutation = useAddPsychologistToHospital();
  ...
  const handleInvite = async () => {
    ...
    try {
      await addPsychologistMutation.mutateAsync({
        hospitalId,
        psychologistId: selectedPsychologist.id,
      });
      onClose();
    } catch (error) {
      console.error("Failed to invite psychologist to hospital:", error);
      setErrorMessage(
        getApiErrorMessage(
          error,
          "Failed to invite psychologist. Please try again.",
        ),
      );
    }
  };
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-3 pt-4">
            <Button
              ...
              disabled={addPsychologistMutation.isPending || !selectedPsychologist}
              className="flex-1 bg-[#5850DE] hover:bg-[#4A42C7]"
            >
              {addPsychologistMutation.isPending ? (
                <>
            ...
            <Button
              onClick={onClose}
              variant="outline"
              disabled={addPsychologistMutation.isPending}
              className="flex-1"
            >
              Cancel
            </Button>
