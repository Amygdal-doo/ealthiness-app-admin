...
import { useAddDietPlanMeal } from "~/hooks/useAuthApi";
import {
  MEAL_TYPES,
  MEAL_TYPE_LABELS,
  type MealType,
...
interface AddMealModalProps {
  ...
  onClose: () => void;
  dietPlanId: string;
  /** Suggested default day, e.g. the last day already in the plan. */
...
export const AddMealModal: React.FC<AddMealModalProps> = ({
  ...
  const addMeal = useAddDietPlanMeal();
  ...
  const resetForm = () => {
    setDayNumber(defaultDayNumber ? String(defaultDayNumber) : "");
    setMealType("");
    setTitle("");
    setDescription("");
    setIngredients([]);
    ...
    try {
      await addMeal.mutateAsync({
        ...
        data: {
          ...
          foodItems: cleanedFoodItems.length > 0 ? cleanedFoodItems : undefined,
      ...
      <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] flex flex-col shadow-2xl animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="p-6 border-t border-[#E0E1E6] flex gap-3 shrink-0">
          <Button
            ...
            disabled={addMeal.isPending || !isValid}
