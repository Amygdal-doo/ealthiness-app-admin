...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  const userData = {
    user: { name: role === 'super_admin' ? "Super Admin" : "Country Admin", role },
    companies: [
      { id: 'comp1', countryId: 'c1', name: 'Sarajevo TechFit', status: 'Active', users: 145 },
      { id: 'comp2', countryId: 'c1', name: 'Mostar Health Corp', status: 'Active', users: 89 },
      { id: 'comp3', countryId: 'c2', name: 'NY Wellness Solutions', status: 'Pending', users: 320 },
      { id: 'comp4', countryId: 'c3', name: 'Berlin BioHacks', status: 'Active', users: 210 }
    ]
  };
...
export default function CompaniesPage({ loaderData }: Route.ComponentProps) {
  ...
  const visibleCompanies = userData.user.role === 'country_admin' 
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FB] font-sans">
      <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-6 p-8">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-2xl font-extrabold text-[#1B173A]">Managed Companies</h2>
            <p className="text-[#60646C] text-sm font-medium mt-1">
              Total {visibleCompanies.length} active companies in your jurisdiction.
            </p>
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead className="bg-[#F8F9FB] border-b border-[#E0E1E6]">
                <tr>
                  <th className="p-4 text-xs font-bold text-[#8E8E93] uppercase tracking-widest">Company Name</th>
                  <th className="p-4 text-xs font-bold text-[#8E8E93] uppercase tracking-widest">Status</th>
                  <th className="p-4 text-xs font-bold text-[#8E8E93] uppercase tracking-widest">Avg Scores</th>
                  <th className="p-4 text-xs font-bold text-[#8E8E93] uppercase tracking-widest">Total Users</th>
                  <th className="p-4 text-xs font-bold text-[#8E8E93] uppercase tracking-widest text-right">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[#E0E1E6]">
                {visibleCompanies.map(comp => (
                  <tr key={comp.id} className="hover:bg-gray-50 transition">
                    ...
                    <td className="p-4">
                      <Badge variant={comp.status === 'Active' ? 'secondary' : 'outline'}>
                        {comp.status}
                      </Badge>
                    </td>
                    <td className="p-4">
                      <div className="flex flex-col gap-1 text-xs font-medium text-[#60646C]">
                        <span className="flex items-center justify-between w-24">
                          Mood: <span className="text-[#1B173A] font-bold">88/100</span>
                        </span>
                        <span className="flex items-center justify-between w-24">
                          Fit: <span className="text-[#1B173A] font-bold">92/100</span>
                        </span>
                      </div>
                    </td>
                    <td className="p-4 font-bold text-[#1B173A]">{comp.users}</td>
                    <td className="p-4 text-right">
                      <div className="flex justify-end gap-2">
                        <Button variant="ghost" className="px-2" title="Edit Company">
                          <Edit size={18} />
                        </Button>
                        <Button 
                          variant="outline" 
            ...
            <Card className="w-full max-w-md shadow-2xl animate-in zoom-in-95 duration-200">
              ...
              <div className="p-6 space-y-4">
                {modalState.type === 'invite_admin' && (
                  <>
                    <p className="text-sm text-[#60646C]">
                      You are inviting a new {modalState.data?.role} to manage <strong>{modalState.data?.entity}</strong>. 
                      They will receive an email to set up their account.
                    </p>
                    <div className="space-y-2">
                      <label className="text-xs font-bold text-[#8E8E93] uppercase">Email Address</label>
                      <input 
                ...
                {modalState.type === 'company' && (
                  <div className="space-y-2">
                    <label className="text-xs font-bold text-[#8E8E93] uppercase">Company Name</label>
                    <input 
                      className="w-full px-3 py-2 border border-[#E0E1E6] rounded-lg focus:border-[#5850DE] outline-none"
                ...
                <div className="pt-4 flex justify-end gap-3">
                  ...
                  <Button 
                    ...
                    onClick={() => setModalState({ isOpen: false, type: '', data: null })}
                  >
                    {modalState.type === 'invite_admin' ? 'Send Invitation' : 'Save'}
                  </Button>
                </div>
              </div>
            </Card>
