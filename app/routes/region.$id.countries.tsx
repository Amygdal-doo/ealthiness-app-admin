...
import type { Route } from "./+types/region.$id.countries";
import {
  Globe,
  Plus,
  Mail,
...
import { useRegionCountries, useRegionDetails } from "~/hooks/useAuthApi";
...
export function meta({}: Route.MetaArgs) {
  return [
    { title: "Region Countries - Ealthiness Admin Portal" },
    {
      name: "description",
      content: "Manage countries in the region",
    },
  ];
}
...
export default function RegionCountriesPage({
  ...
  // Fetch region countries with React Query
  const {
    data: countriesResponse,
    isLoading,
    error,
    refetch,
  } = useRegionCountries(actualRegionId, {
    page: currentPage,
    limit: 10,
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-[#60646C] text-sm font-medium mt-1">
                    {countriesResponse
                      ? `Total ${countriesResponse.total} countries in this region.`
                      : `Total ${visibleCountries.length} countries in this region.`}
                  </p>
                </div>
              </div>
